...
def match_agencies_to_organization(
    org_fields: Dict[str, Any], agencies_by_name: Dict[str, List[Dict[str, Any]]]
) -> List[Dict[str, Any]]:
...
def match_projects_to_organization(
    org_name: str, org_id: str, projects_by_org: Dict[str, List[Dict[str, Any]]]
) -> List[Dict[str, Any]]:
...
def main():
    ...
    for a in agencies:
        ...
        if not owner:
            owner = (
                fields.get("Org Full Name") or fields.get("Organization Name") or None
            )
        if owner:
    ...
    for p in projects:
        ...
        prov = (
            fields.get("Provider Orgs Full Name")
            or fields.get("Provider Org")
    ...
    for o in orgs:
        ...
        org_name = (
            ofields.get("Org Full Name")
            or ofields.get("Org Short Name")
            or ofields.get("Organization")
            or o.get("id")
        )

        # attach agencies matching by Org Donor Agencies mapping first
        donor_agencies_raw = (
            ofields.get("Org Donor Agencies")
            or ofields.get("Org Donor Agencies (Linked)")
            or ofields.get("Org Donor Agencies (from Agency)")
        )
        ...
        if not attached_agencies:
            ...
            attached_agencies = (
                agencies_by_org.get(org_name or "")
                or agencies_by_org.get(org_id_str)
                or []
            )
        ...
        org_id_str = str(o.get("id", "")) if o.get("id") else ""
        projs = (
            projects_by_org.get(org_name or "") or projects_by_org.get(org_id_str) or []
        )
        ...
        entry = {
            ...
            "donor_countries": sorted(
                list(donor_countries)
            ),  # New field with deduplicated country names
        }

 ...
 */
export function generateOrganizationsCSV(
    ...
    org.projects.forEach((project) => {
      ...
      if (assetMap.has(assetKey)) {
        ...
        project.investmentTypes.forEach((type) =>
          existing.investmentTypes.add(type),
        );
...
export async function exportViewAsXLSX(
  ...
  try {
    ...
    aboutText.getCell(1).value =
      "This export contains data from the Crisis Data Funding Compass, an overview of the crisis data funding ecosystem. The dataset includes information about organizations providing crisis data products and services, along with details about their specific assets/projects. The data set is subject to expansion and correction.";
    ...
    if (hasFilters) {
      ...
      filtersRow.getCell(1).value =
        "This export represents a filtered view of the data:";
      ...
      if (
        filterInfo?.investmentTypes &&
        filterInfo.investmentTypes.length > 0
      ) {
        const row = readmeSheet.getRow(rowIndex++);
      ...
      if (
        filterInfo?.investmentThemes &&
        filterInfo.investmentThemes.length > 0
      ) {
        const row = readmeSheet.getRow(rowIndex++);
      ...
      const filtersRow = readmeSheet.getRow(rowIndex++);
      filtersRow.getCell(1).value =
        "This export contains the complete dataset with no filters applied.";
      filtersRow.getCell(1).font = { name: "Roboto", size: 11 };
    }
    ...
    [
      ...
      [
        "Unique Donor Countries:",
        new Set(organizations.flatMap((org) => org.donorCountries)).size,
      ],
    ].forEach(([label, value]) => {
      const row = readmeSheet.getRow(rowIndex++);
    ...
    sourceRow1.getCell(1).value =
      "This data is maintained by the Complex Risk Analytics Fund (CRAF'd).";
    ...
    sourceRow3.getCell(1).value =
      "For questions about this dataset, please contact CRAF'd through the feedback form in the Crisis Data Funding Compass.";

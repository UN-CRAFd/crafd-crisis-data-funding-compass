...
import type { DashboardStats, ProjectTypeData, OrganizationTypeData, OrganizationWithProjects } from '@/types/airtable';
...
export async function exportDashboardToPDF({
    ...
    const drawBarChart = (
        ...
        data.forEach((item, index) => {
            ...
            pdf.text(item.value.toString(), barStartX + maxBarWidth + 2, currentY + 3.2);
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setTextColor(brandPrimaryDark);
    yPosition = addWrappedText(filterDesc, margin + 3, yPosition + 4.5, contentWidth - 6, 4.5);
    yPosition += 8;
    ...
    const statCards = [
        { label: 'Donors', value: stats.donorCountries, subtitle: 'Where funds are from' },
        { label: 'Organizations', value: stats.dataProviders, subtitle: 'Who receives funds' },
        { label: 'Assets', value: stats.dataProjects, subtitle: 'What funds are used for' }
    ];
    ...
    statCards.forEach((card, index) => {
        ...
        pdf.roundedRect(cardX + 0.5, yPosition + 0.5, cardWidth, cardHeight, 3, 3, 'F');
        ...
        pdf.setFont('helvetica', 'bold');
        pdf.setTextColor(brandPrimaryDark);
        pdf.text(card.value.toString(), cardX + cardWidth / 2, yPosition + 13, { align: 'center' });

        // Label
        pdf.setFontSize(10);
        pdf.setFont('helvetica', 'bold');
        pdf.setTextColor(textDark);
        pdf.text(card.label, cardX + cardWidth / 2, yPosition + 19, { align: 'center' });
        ...
        pdf.text(subtitleLines, cardX + cardWidth / 2, yPosition + 23, { align: 'center' });
    });

...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const urlDonorSlugs = incomingDonorSlugs.flatMap((s) =>
        s === "crafd-donors" ? crafdExpansion : [s],
      );

      const rawTypes = searchParams.get("types") ?? searchParams.get("t") ?? "";
      const urlTypes = rawTypes
        .split(",")
        .map((s) => decodeURIComponent(s))
        .filter(Boolean);

      const rawThemes =
        searchParams.get("th") ?? searchParams.get("themes") ?? "";
      const themeKeys = rawThemes
        .split(",")
        .map((s) => decodeURIComponent(s))
        .filter(Boolean);
  ...
  useEffect(() => {
    ...
    if (urlDonorSlugs && urlDonorSlugs.length > 0) {
      ...
      const decodedDonors = urlDonorSlugs
        .map((slug: string) =>
          availableDonorCountries.find((d) => matchesUrlSlug(slug, d)),
        )
        .filter(Boolean);
  ...
  useEffect(() => {
    ...
    if (investmentThemes.length > 0) {
      ...
      const keys = Array.from(
        new Set(investmentThemes.map((t) => themeNameToKey(t))),
      );
  ...
  const filteredOrganizationsData = useMemo(() => {
    ...
    return organizationsData.filter((org) => {
      ...
      if (appliedSearchQuery) {
        ...
        const orgType = (org.fields?.["Org Type"] || "")
          .toString()
          .toLowerCase();
        const matchesSearch =
          orgName.includes(query) || orgType.includes(query);
      ...
      if (investmentThemes.length > 0) {
        const hasMatchingProject = org.projects?.some((project) => {
          ...
          return (
            Array.isArray(projectThemes) &&
            projectThemes.some((theme) =>
  ...
  const coFinancingMatrix = useMemo(() => {
    const matrix: Record<
      string,
      Record<string, { projects: Set<string>; orgs: Set<string> }>
    > = {};
      ...
      selectedDonors.forEach((donor1, i) => {
        selectedDonors.forEach((donor2, j) => {
          if (i !== j) {
            ...
            const hasBothDonors =
              allOrgDonors.includes(donor1) && allOrgDonors.includes(donor2);

            if (hasBothDonors) {
              // Check if org has any projects that match type/theme filters
              let hasMatchingProjects = true;
              if (
                (investmentTypes.length > 0 || investmentThemes.length > 0) &&
                org.projects &&
                ...
                hasMatchingProjects = org.projects.some((project) => {
                  ...
                  if (investmentTypes.length > 0) {
                    const projectTypes =
                      project.fields?.["Investment Type(s)"] || [];
                    const matchesType =
                      Array.isArray(projectTypes) &&
                      projectTypes.some((type) =>
                        investmentTypes.some(
                  ...
                  if (investmentThemes.length > 0) {
                    const projectThemes =
                      project.fields?.["Investment Theme(s)"] || [];
                    const matchesTheme =
                      Array.isArray(projectThemes) &&
                      projectThemes.some((theme) =>
                        investmentThemes.some(
              ...
              if (org.projects && Array.isArray(org.projects)) {
                org.projects.forEach((project) => {
                  ...
                  const projectName =
                    project.fields?.["Project Name"] || project.name || "";
  ...
  const maxValues = useMemo(() => {
    ...
    selectedDonors.forEach((donor1) => {
      selectedDonors.forEach((donor2) => {
        if (donor1 !== donor2) {
          const projectCount =
            coFinancingMatrix[donor1]?.[donor2]?.projects.size || 0;
  ...
  const organizationTypesData = useMemo(() => {
    ...
    console.log(
      ...
      typeCount,
      "Total orgs:",
      filteredOrganizationsData.length,
    );
  ...
  const organizationsWithDonorCounts = useMemo(() => {
    ...
    const orgDonorMap: Record<
      string,
      { name: string; donors: Set<string>; id: string }
    > = {};
      ...
      if (org.agencies && Array.isArray(org.agencies)) {
        org.agencies.forEach((agency) => {
          ...
          if (
            ...
            selectedDonors.includes(countryName)
          ) {
            allDonorsSet.add(countryName);
          }
        });
    ...
    return Object.values(orgDonorMap).sort((a, b) => {
  ...
  const donorProfiles = useMemo(() => {
    ...
    const projectMatchesFilters = (project: any) => {
      ...
      if (investmentTypes.length > 0) {
        ...
        if (
          ...
          !projectTypes.some((pt: string) =>
            investmentTypes.some(
              (it) =>
                it.toLowerCase().trim() === (pt || "").toLowerCase().trim(),
            ),
          )
        ) {
          return false;
      ...
      if (investmentThemes.length > 0) {
        ...
        if (
          !Array.isArray(projectThemes) ||
          !projectThemes.some((th: string) =>
            investmentThemes.some(
              (it) =>
                it.toLowerCase().trim() === (th || "").toLowerCase().trim(),
            ),
          )
        ) {
          return false;
    ...
    return selectedDonors.map((donor) => {
            ...
            if (projectDonors.has(donor) && projectMatchesFilters(project)) {
              ...
              if (Array.isArray(projectTypes) && projectTypes.length > 0) {
                projectTypes.forEach((pt: string) => {
                  const match = allKnownInvestmentTypes.find(
                    (k) =>
                      k.toLowerCase().trim() ===
                      (pt || "").toLowerCase().trim(),
                  );
      ...
      const data = allKnownInvestmentTypes.map((t) => ({
        type: t,
        value: counts[t] || 0,
      }));
      return { donor, data };
  ...
  const donorCoFinancingData = useMemo(() => {
    ...
    return pairCounts.sort((a, b) => b.value - a.value).slice(0, 10);
  }, [coFinancingMatrix, selectedDonors]);
  ...
  const analyticsStats = useMemo(() => {
    ...
    selectedDonors.forEach((country) => {
      ...
      organizationsData.forEach((org) => {
        ...
        const hasCountryAsOrgDonor = org.agencies?.some(
          (agency) => agency.fields?.["Country Name"] === country,
        );
      ...
      organizationsData.forEach((org) => {
        if (org.projects && Array.isArray(org.projects)) {
          org.projects.forEach((project) => {
            ...
            const hasCountryAsProjectDonor = project.agencies?.some(
              (agency: any) => agency.fields?.["Country Name"] === country,
            );
    ...
    const orgsWithTotalDonorCounts: Array<{
      orgId: string;
      totalDonors: Set<string>;
    }> = [];
    ...
    const avgDonorsPerOrg =
      orgsWithTotalDonorCounts.length > 0
        ? Number(
            (
              orgsWithTotalDonorCounts.reduce(
                (sum, org) => sum + org.totalDonors.size,
                0,
              ) / orgsWithTotalDonorCounts.length
            ).toFixed(1),
          )
        : 0;
      ...
      <div className="mx-auto max-w-[82rem] px-4 py-0 pt-20 sm:px-6 sm:py-0 sm:pt-24 lg:px-8">
        <div className="space-y-4 sm:space-y-4">
              ...
              {showGeneralContributions && (
                <div className="mt-4 flex gap-3 rounded-lg border border-[var(--brand-primary-light)]/40 bg-white/60 p-4 backdrop-blur-sm">
                  ...
                  <p className="text-sm text-amber-900">
                    ...
                    deviate from the dashboard page if General Contributions are
                    turned on.
                  </p>
                </div>
              )}
          ...
          {selectedDonors.length > 0 && (
            <>
              {/* Desktop Grid */}
              <div className="hidden gap-4 sm:grid sm:grid-cols-2 sm:gap-[var(--spacing-section)] lg:grid-cols-4">
                <StatCard
                ...
                <StatCard
                  ...
                  value={
                    selectedDonors.length === 1
                      ? "N/A"
                      : analyticsStats.sharedFundingTargets
                  }
                  label="orgs"
                  colorScheme="amber"
                ...
                <StatCard
                  ...
                  value={
                    selectedDonors.length === 1
                      ? "N/A"
                      : `${analyticsStats.avgFundingOverlap}%`
                  }
                  label="shared"
                  colorScheme="amber"
                  tooltip={
                    <div className="space-y-2">
                      <p className="text-sm font-medium">
                        Percentage of targets funded by exactly one selected
                        donor:
                      </p>
                        ...
                        <tbody>
                          <tr>
                            ...
                            <td className="text-right font-semibold">
                              {analyticsStats.fundingOverlapDetails
                                ?.totalTargets || 0}
                            </td>
                          </tr>
                          <tr>
                            <td className="py-1 pr-3 text-left">
                              Funded by exactly 1 donor:
                            </td>
                            <td className="text-right font-semibold">
                              {analyticsStats.fundingOverlapDetails
                                ?.uniqueFundingTargets || 0}
                            </td>
                          </tr>
                          <tr>
                            <td className="py-1 pr-3 text-left">
                          ...
                          <tr className="mt-1 border-t border-gray-300 pt-1">
          ...
          <Card className="!border-0 bg-white">
            ...
            <CardContent className="mt-0 mb-0 p-4">
              <FilterBar
          ...
          >
            <CardHeader className="pb-0">
              <div className="flex items-center justify-between gap-4">
                <CardTitle>
                  <SectionHeader
                    icon={<Network style={{ color: "var(--brand-primary)" }} />}
                ...
                <>
                  <div className="mb-3 flex items-center gap-4">
                    <div className="ml-auto flex items-center gap-3">
                      <div className="flex items-center gap-1.5">
                        <div className="h-4 w-4 rounded border border-slate-300 bg-amber-300"></div>
                        <span className="text-xs text-slate-600">
                          Organizations
                        </span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <div className="h-4 w-4 rounded border border-slate-300 bg-indigo-300"></div>
                  ...
                  <div className="w-full overflow-x-hidden">
                    <table className="w-full">
                      <thead>
                        <tr>
                          <th className="min-w-[90px] border-b-2 border-slate-200 p-2 text-left text-sm font-semibold text-slate-600">
                            Donor
                          </th>
                          {selectedDonors.map((donor) => (
                            <th
                              key={donor}
                              className="w-16 border-b-2 border-slate-200 p-2 text-left text-sm font-semibold text-slate-600"
                              style={{
                      ...
                      <tbody>
                        {selectedDonors.map((donor1, i) => (
                          <tr key={donor1}>
                            <td className="border-r-2 border-slate-200 p-2 text-sm font-semibold text-slate-600">
                              {donor1}
                            </td>
                            {selectedDonors.map((donor2, j) => {
                              ...
                              const projectCount =
                                coFinancingMatrix[donor1]?.[donor2]?.projects
                                  .size || 0;
                              const orgCount =
                                coFinancingMatrix[donor1]?.[donor2]?.orgs
                                  .size || 0;

                              const count = isAboveDiagonal
                                ? projectCount
                                : orgCount;

                              const showDiagonalContext =
                                isDiagonal &&
                                hoveredCell &&
                                (hoveredCell.donor1 === donor1 ||
                                  hoveredCell.donor2 === donor1);
                              let diagonalCount = 0;
                              let diagonalColorClass =
                                "bg-slate-200 text-slate-400";
                              let diagonalTooltipText = "";

                              if (showDiagonalContext) {
                                const hoveredOrgIndex = selectedDonors.indexOf(
                                  hoveredCell.donor1,
                                );
                                const hoveredColIndex = selectedDonors.indexOf(
                                  hoveredCell.donor2,
                                );
                                const isHoveredCellProject =
                                  hoveredOrgIndex < hoveredColIndex;

                                if (isHoveredCellProject) {
                                  diagonalCount =
                                    donorTotals[donor1]?.projects.size || 0;
                                  ...
                                  diagonalCount =
                                    donorTotals[donor1]?.orgs.size || 0;
                              ...
                              const cellContent = (
                                <td
                                  ...
                                  onClick={() =>
                                    !isDiagonal &&
                                    count > 0 &&
                                    handleCellClick(donor1, donor2)
                                  }
                                  className={`border border-slate-200 p-3 text-center text-sm font-semibold transition-all ${colorClass} ${!isDiagonal && count > 0 ? "cursor-pointer hover:opacity-75" : ""}`}
                                >
                                  {isDiagonal && showDiagonalContext
                                    ? diagonalCount
                                    : isDiagonal
                            ...
                            <tr key={donor1}>
                              ...
                              {selectedDonors.map((donor2, j) => {
                                ...
                                const cellContent = (
                                  <td
                                    key={donor2}
                                    className={`border border-slate-200 p-3 text-center text-sm font-semibold transition-all ${colorClass} ${!isDiagonal && orgCount > 0 ? "cursor-pointer hover:opacity-75" : ""}`}
                                    onClick={() =>
                                      !isDiagonal &&
                                      orgCount > 0 &&
                                      handleCellClick(donor1, donor2)
                            ...
                            <tr key={donor1}>
                              ...
                              {selectedDonors.map((donor2, j) => {
                                ...
                                const cellContent = (
                                  <td
                                    key={donor2}
                                    className={`border border-slate-200 p-3 text-center text-sm font-semibold transition-all ${colorClass} ${!isDiagonal && projectCount > 0 ? "cursor-pointer hover:opacity-75" : ""}`}
                                    onClick={() =>
                                      !isDiagonal &&
                                      projectCount > 0 &&
                                      handleCellClick(donor1, donor2)
            ...
            <Card className="hidden !border-0 bg-white">
              <CardHeader className="pb-2">
                <div className="flex flex-col gap-1">
                  <SectionHeader
                    icon={
                      <BarChart3 style={{ color: "var(--badge-other-icon)" }} />
                    }
              ...
              <CardContent className="pt-2">
                <div className="w-full">
                  <div className="flex flex-col items-start gap-6 lg:flex-row">
                    ...
                    <div className="h-[640px] min-w-0 flex-1 lg:h-[720px]">
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          <PolarAngleAxis
                            dataKey="name"
                            tick={({ x, y, payload }) => {
                              const Icon = getIconForInvestmentType(
                                payload.value,
                              );
                          ...
                          {selectedDonors.map((donor, index) => {
                            ...
                            const color =
                              purplePalette[index % purplePalette.length];
                            const seriesOpacity =
                              hoveredDonor === null
                                ? 1
                                : hoveredDonor === donor
                    ...
                    <div className="w-full flex-shrink-0 lg:w-56">
                      <div className="sticky top-8 flex flex-col gap-2">
                        {selectedDonors.map((donor, idx) => {
                          const isActive =
                            hoveredDonor === null || hoveredDonor === donor;
                          const baseColor = [
                            "#8b5cf6",
                            "#7c3aed",
                            "#6d28d9",
                          ...
                          return (
                            <div
                              ...
                              className={`flex cursor-pointer items-center gap-3 rounded-md border p-2 transition-all ${isActive ? "bg-white shadow-sm" : "bg-slate-50 opacity-60"}`}
                            >
                              <div
                              ...
                              <div className="min-w-0">
                                ...
                                <div className="truncate text-xs text-slate-500">
                                  {donorTotals[donor]?.projects.size || 0}{" "}
                                  projects
                                </div>
                              </div>
                            </div>
                ...
                <p className="mt-2 text-xs text-slate-500">
                  Values are normalized per donor. Percentages represent
                  relative focus, not absolute volume.
                </p>
              </CardContent>
            </Card>
          )}
          ...
          {/* Charts Row */}
          <div
            className={`${selectedDonors.length === 0 ? "hidden" : "hidden"} grid grid-cols-1 gap-4 lg:grid-cols-2`}
          >
            <div>
              <ChartCard
            ...
            <Card className="!border-0 bg-white">
              <CardHeader className="pb-3">
                <CardTitle>
                  <SectionHeader
                    ...
                    title="Co-Financed Organizations"
                  />
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-0">
                <div className="max-h-70 space-y-1 overflow-y-auto">
                  {organizationsWithDonorCounts.length === 0 ? (
                    ...
                    organizationsWithDonorCounts.map((org, idx) => (
                      <div
                        key={`${org.id}-${idx}`}
                        className="flex items-start justify-between gap-2 rounded px-2 py-1.5 text-xs transition-colors hover:bg-slate-50"
                      >
                ...
                <p className="mt-2 border-t border-slate-100 pt-2 text-xs text-slate-500">
                  {organizationsWithDonorCounts.length} org
                  {organizationsWithDonorCounts.length !== 1 ? "s" : ""}
                </p>
              </CardContent>
            </Card>
          </div>

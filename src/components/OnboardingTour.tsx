...
export default function OnboardingTour({ forceOpen }: { forceOpen?: boolean }) {
  ...
  const steps: Step[] = [
    ...
    },
    {
      ...
      title: "You are ready!",
      description:
        "That covers the essentials. You can re-open this tour from the help menu later.",
      ...
      <div className="relative mx-4 w-full max-w-3xl">
        <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-lg">
            ...
            <div className="mt-6 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`rounded-md border px-3 py-1 ${stepIndex === 0 ? "cursor-not-allowed opacity-40" : "bg-slate-50 hover:bg-slate-100"}`}
                >
                  Prev
                </button>
                ...
                <div className="flex items-center gap-2">
                  {steps.map((s, i) => (
                    <button
                      ...
                      className={`h-2 w-2 rounded-full ${i === stepIndex ? "bg-[var(--brand-primary)]" : "bg-slate-200"}`}
                      aria-label={`step-${i + 1}`}
                    />
                  ))}
                </div>
              ...
              <div className="flex items-center gap-3">
                ...
                {stepIndex < steps.length - 1 ? (
                  <button
                    ...
                    className="rounded-md bg-[var(--brand-primary)] px-4 py-2 text-white shadow-sm hover:opacity-95"
                  >
                    Next
                  </button>
                ) : (
          ...
          <div className="border-t border-slate-100 bg-slate-50 px-6 py-3 text-xs text-slate-500">
            Tip: you can re-open this tour from the help menu if you change your
            mind.
          </div>
        </div>
      </div>

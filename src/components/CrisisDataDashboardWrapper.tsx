...
const CrisisDataDashboardWrapper = () => {
  ...
  const updateFilterParams = useCallback(
    (params: {
      ...
      if (params.donors !== undefined && params.donors.length > 0) {
        newSearchParams.set(
          "d",
          params.donors.map((d) => toUrlSlug(d)).join(","),
        );
      ...
      if (params.types !== undefined && params.types.length > 0) {
        ...
        ).map((s) => slugs.find((slug) => slug.toLowerCase() === s) || s);
      ...
      if (params.themes !== undefined && params.themes.length > 0) {
        newSearchParams.set(
          "th",
          params.themes.map((t) => themeNameToKey(t)).join(","),
        );
      } else if (params.themes === undefined && investmentThemes.length > 0) {
        newSearchParams.set(
          "th",
          investmentThemes.map((t) => themeNameToKey(t)).join(","),
        );
      }
    ...
    async function fetchData() {
      try {
        ...
        const filters: DashboardFilters = {
          ...
          investmentThemes:
            investmentThemes.length > 0 ? investmentThemes : undefined,
  ...
  const handleSortChange = (
    newSortBy: "name" | "donors" | "assets" | "funding",
  ...
  const handleOpenDonorModal = useCallback(
    (donorCountry: string) => {

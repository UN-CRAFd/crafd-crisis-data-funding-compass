...
export default function OrganizationModal({ organization, loading }: OrganizationModalProps): React.ReactElement {
  ...
  // Precompute description and HQ country blocks with proper type narrowing
  let descriptionBlock: React.ReactNode | any = null;
  if (typeof fields['Org Description'] === 'string' && String(fields['Org Description']).length > 0) {
    descriptionBlock = (
      <div>
        <SubHeader>Description</SubHeader>
        <p className="text-gray-700 leading-relaxed text-sm">{renderValue(String(fields['Org Description']))}</p>
